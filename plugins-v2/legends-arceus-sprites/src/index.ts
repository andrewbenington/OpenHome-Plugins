import { FormeMetadata, Generation, Region, SpeciesMetadata } from '../../../types/pkmrs.d.js'
...
const allFormesAllowed = [NationalDex.Vulpix, NationalDex.Ninetales, NationalDex.Sneasel]
...
export const plugin: SpritePlugin = {
  ...
  getMonSpritePath: (params: MonData) => {
    ...
    if (!formeData || formeData.introduced > Generation.G8) return null

    if (allFormesAllowed.includes(dexNum)) {
      return `assets/${isShiny ? 'shiny/' : ''}${formeData.formeName.toLowerCase()}.png`
    }

    if (formeData.regional !== null && formeData.regional !== Region.Hisui) {
      return null
    }

    if (
      speciesData.formes.some((otherForme) => otherForme.regional === Region.Hisui) &&
      formeData.regional !== Region.Hisui
    ) {
      return null
    }
